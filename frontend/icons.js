// Shared inline SVG icons (no external dependency / CDN needed)
const ICONS = {
    ...
    ranking: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="20" x2="12" y2="10"/><line x1="18" y1="20" x2="18" y2="4"/><line x1="6" y1="20" x2="6" y2="16"/></svg>`,
...
function buildNav(active) {
    const links = [
        ...
        { href: "candidates.html", label: "Candidates", icon: "users" },
        { href: "ranking.html", label: "Ranking", icon: "ranking" },
        { href: "job_match.html", label: "Job Match", icon: "target" },
        { href: "ai_tools.html", label: "AI Tools", icon: "sparkles" },
    ];

    return links.map(link => {
        const isActive = link.href === active ? "active" : "";
        return `<a href="${link.href}" class="${isActive}">${ICONS[link.icon]}<span>${link.label}</span></a>`;
    }).join("");
}
...
function injectHeader(active) {
    ...
    header.innerHTML = `
        <h1><a href="landing.html">${ICONS.logo}AI <span>Recruitment</span> Platform</a></h1>
        <nav class="tabs">${buildNav(active)}</nav>
    `;
}
