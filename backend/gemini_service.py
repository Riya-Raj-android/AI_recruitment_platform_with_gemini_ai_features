...
def _call_gemini(prompt: str) -> str:
    ...
    Retries once if the model is temporarily overloaded (503)."""
    ...
    for attempt in range(2):
        ...
        if response.status_code == 503 and attempt == 0:
            last_error = "Model temporarily overloaded, retrying..."
            time.sleep(2)
            continue
        ...
        if response.status_code == 429:
            return (
                "The AI service has hit its usage limit for now. "
                "Please wait a bit before trying again."
            )

        return f"ERROR: Gemini API returned {response.status_code} - {response.text[:300]}"

    return last_error or "ERROR: Unknown failure calling Gemini API"
